...
kwbase::roachpb::ValueType GetTag(const std::string& val) {
  if (val.size() < kHeaderSize) {
    return kwbase::roachpb::UNKNOWN;
  }
  return kwbase::roachpb::ValueType(val[kTagPos]);
}

void SetTag(std::string* val, kwbase::roachpb::ValueType tag) { (*val)[kTagPos] = tag; }
...
void SerializeProtoToValue(std::string* val, const google::protobuf::MessageLite& msg) {
  ...
  SetTag(val, kwbase::roachpb::BYTES);
  msg.AppendToString(val);
}

// Method used to sort InternalTimeSeriesSamples.
bool TimeSeriesSampleOrdering(const kwbase::roachpb::InternalTimeSeriesSample* a,
                              const kwbase::roachpb::InternalTimeSeriesSample* b) {
...
bool IsTimeSeriesData(const std::string& val) {
  return GetTag(val) == kwbase::roachpb::TIMESERIES;
}

void SerializeTimeSeriesToValue(std::string* val,
                                const kwbase::roachpb::InternalTimeSeriesData& ts) {
  SerializeProtoToValue(val, ts);
  SetTag(val, kwbase::roachpb::TIMESERIES);
}
...
WARN_UNUSED_RESULT bool MergeTimeSeriesValues(std::string* left, const std::string& right,
  ...
  kwbase::roachpb::InternalTimeSeriesData left_ts;
  kwbase::roachpb::InternalTimeSeriesData right_ts;
  ...
  if (use_column_format) {
    ...
    kwbase::roachpb::InternalTimeSeriesData new_ts;
    ...
    while (left_front != left_end || right_front != right_end) {
      ...
      kwbase::roachpb::InternalTimeSeriesSample* ns = new_ts.add_samples();
      ...
      kwbase::roachpb::InternalTimeSeriesSample src;
...
WARN_UNUSED_RESULT bool ConsolidateTimeSeriesValue(std::string* val, rocksdb::Logger* logger) {
  // Attempt to parse TimeSeriesData from both Values.
  kwbase::roachpb::InternalTimeSeriesData val_ts;
  ...
  if (val_ts.offset_size() > 0) {
    ...
    using sample = kwbase::roachpb::InternalTimeSeriesSample;
    auto it =
...
class DBMergeOperator : public rocksdb::MergeOperator {
  virtual const char* Name() const { return "kwbase_merge_operator"; }
  ...
  virtual bool FullMerge(const rocksdb::Slice& key, const rocksdb::Slice* existing_value,
    ...
    kwbase::storage::enginepb::MVCCMetadata meta;
 ...
 private:
  bool MergeOne(kwbase::storage::enginepb::MVCCMetadata* meta,
                const rocksdb::Slice& operand, bool full_merge,
                rocksdb::Logger* logger) const WARN_UNUSED_RESULT {
    kwbase::storage::enginepb::MVCCMetadata operand_meta;
...
void convertToColumnar(kwbase::roachpb::InternalTimeSeriesData* data) {
...
void sortAndDeduplicateColumns(kwbase::roachpb::InternalTimeSeriesData* data,
                               int first_unsorted) {
...
WARN_UNUSED_RESULT bool MergeValues(kwbase::storage::enginepb::MVCCMetadata* left,
                                    const kwbase::storage::enginepb::MVCCMetadata& right,
                                    bool full_merge, rocksdb::Logger* logger) {
...
DBStatus MergeResult(kwbase::storage::enginepb::MVCCMetadata* meta, DBString* result) {
