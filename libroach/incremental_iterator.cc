...
bool DBIncrementalIterator::legacyTimestampIsLess(const kwbase::util::hlc::LegacyTimestamp& t1,
                                                  const kwbase::util::hlc::LegacyTimestamp& t2) {
  ...
  for (;;) {
    ...
    if (meta.has_txn()) {
      if (legacyTimestampIsLess(start_time, meta.timestamp()) &&
        ...
        kwbase::roachpb::WriteIntentError err;
        kwbase::roachpb::Intent* intent = err.add_intents();
